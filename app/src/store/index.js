...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async getDeckList({ commit }) {
      try {
        const request = await axios.get('https://storage.googleapis.com/ko-decks/decklist.json')
    ...
    async deckChange({ commit, dispatch, state }, deck) {
      ...
      if (!deck.downloaded) {
        ...
        const request = await axios.get(`https://storage.googleapis.com/ko-decks/${filename}`, {
