...
const fullDecksBucket = 'ko-decks-full'
const fullDecksBucketLS = 'spf-full-decks'
const decksBucket = 'ko-decks'
...
async function getCards(deckDataSpreadsheet, deck) {
    ...
    await deckSheet.loadCells(`A1:C200`);

    let prevCards = {}
    let prevCardsLS = {}
    try {
        let fileDataLS = await storage.bucket(fullDecksBucketLS).file(deck.nameNS + '.json').download()
    ...
    for (let rowIdx=0; rowIdx<200; rowIdx++) {
        ...
        const questionS = deckSheet.getCell(rowIdx, 1).value
        const question = deckSheet.getCell(rowIdx, 2).value
        let card = {}
        let cardStructure = JSON.parse(cardStructureJSON)
        for (let key in cardStructure) {
            card[key] = cardStructure[key]
        }
        if (prevCardsLS[questionS]) {
            card.images = prevCardsLS[questionS].images
        }
        if (prevCards[question]) {
            card.audio = prevCards[questionS].audio
        }
